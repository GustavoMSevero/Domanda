app.controller("ProfissionaisCtrl", ['$scope', '$http', '$window', '$location', '$rootScope', function ($scope, $http, $window, $location, $rootScope) {
	...
	var getPro = function(){
		...
		//$http.get("http://localhost:8888/sistemas/Android/areaAdmin/api/admin_estabelecimento/profissionais.php?opcao="+opcao).success(function(response){
		$http.get("http://reservacomdomanda.com/areaAdmin/api/admin_estabelecimento/profissionais.php?opcao="+opcao).success(function(response){
